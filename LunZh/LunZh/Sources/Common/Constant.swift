import UIKit

struct Constants {
    
    struct Colors {
        static let red20 = UIColor(named: "red20")
        static let red40 = UIColor(named: "red40")
        static let red60 = UIColor(named: "red60")
        static let red80 = UIColor(named: "red80")
        static let red100 = UIColor(named: "red100")
        static let white = UIColor(named: "white")
        static let yellow20 = UIColor(named: "yellow20")
        static let yellow40 = UIColor(named: "yellow40")
        static let yellow60 = UIColor(named: "yellow60")
        static let yellow80 = UIColor(named: "yellow80")
        static let yellow100 = UIColor(named: "yellow100")
        static let gray100 = UIColor(named: "gray100")
        static let gray200 = UIColor(named: "gray200")
        static let gray300 = UIColor(named: "gray300")
        static let gray400 = UIColor(named: "gray400")
        static let gray500 = UIColor(named: "gray500")
        static let gray600 = UIColor(named: "gray600")
        static let gray700 = UIColor(named: "gray700")
        static let gray800 = UIColor(named: "gray800")
    }
    
    static let TasteList = [
        // 한식
        "김치찌개",
        "불고기",
        "비빔밥",
        "떡볶이",
        "삼겹살",
        
        // 중식
        "짜장면",
        "탕수육",
        "짬뽕",
        "마파두부",
        "볶음밥",
        
        // 양식
        "파스타",
        "피자",
        "햄버거",
        "스테이크",
        "리조또",
        
        // 일식
        "스시",
        "라멘",
        "돈카츠",
        "우동",
        "오코노미야키"
    ]
    
    static let superViewHeight = UIScreen.main.bounds.height
    static let superViewWidth = UIScreen.main.bounds.width
}
